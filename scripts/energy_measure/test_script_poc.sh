            ...
            for type in "${executable_type[@]}"; do
                ...
                if [ -f "$executable_path" ]; then
                    ...
                    if [ "$AA_or_DNA" = "AA" ]; then
                        ...
                        perf-report --no-mpi --report-file perf_report_${ex_type}_${UNIQUE_NAME} $executable_path -s alignment_${length}.phy -te tree_${i}.full.treefile --seqtype AA -prefix outputncu_${UNIQUE_NAME}_${taxa_size}_${length}_aa_${type}.txt
                    elif [ "$AA_or_DNA" = "DNA" ]; then
                        echo "Using DNA data"
                         perf-report --no-mpi --report-file perf_report_${ex_type}_${UNIQUE_NAME} $executable_path -s alignment_${length}.phy -te tree_${i}.full.treefile -prefix outputncu_${UNIQUE_NAME}_${taxa_size}_${length}_dna_${type}.txt
                    fi
