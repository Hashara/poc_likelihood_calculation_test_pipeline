      ...
      if [ "$TYPE" == "OPENACC" ] || [ "$TYPE" == "OPENACC_PROFILE" ] || [ "$TYPE" == "CUDA" ]; then
          ...
          if [ "$V100_GPU" == true ]; then
            memory=$((mem_factor * 1 * 48))
              ...
              qsub -P${PROJECT_NAME} -lwalltime=$wall_time,ncpus=12,ngpus=1,mem="${memory}GB",jobfs=50GB,wd -qgpuvolta -N profile_v100_${TYPE} \
          ...
          if [ "$A100_GPU" == true ]; then
            memory=$(echo "$mem_factor * 0.5 * 64" | bc)
             ...
             qsub -P${PROJECT_NAME} -lwalltime=$wall_time,ncpus=16,ngpus=1,mem="64GB",jobfs=50GB,wd -qdgxa100 -N profile_a100_${TYPE} \
                    -v ARG1,ARG2,ARG3,ARG4,ARG5,ARG6,ARG7,ARG8 "$WD"/profile/iqtree/test_script_iqtree.sh
          fi
          ...
          if [ "$H200" == true ]; then
            memory=$((mem_factor * 1 * 48))
             ...
             qsub -P${PROJECT_NAME} -lwalltime=$wall_time,ncpus=12,ngpus=1,mem="${memory}GB",jobfs=50GB,wd -qgpuhopper -N profile_h200_${TYPE} \
                    -v ARG1,ARG2,ARG3,ARG4,ARG5,ARG6,ARG7,ARG8 "$WD"/profile/iqtree/test_script_iqtree.sh
          fi
         ...
         qsub -P${PROJECT_NAME} -lwalltime=$wall_time,ncpus=1,mem="${memory}GB",jobfs=50GB,wd -qnormal -N profile_iqtree_${TYPE} \
                -v ARG1,ARG2,ARG3,ARG4,ARG5,ARG6,ARG7,ARG8 "$WD"/profile/iqtree/test_script_iqtree.sh
      fi
  done
done
