  ...
  for data_type in "${data_types[@]}"; do
          ...
          if [ "$V100_GPU" == true ]; then
            memory=$((factor * 1 * 48))
              qsub -P${PROJECT_NAME} -lwalltime=$wall_time,ncpus=12,ngpus=1,mem="${memory}GB",jobfs=10GB,wd -qgpuvolta -N energy_v100_${TYPE} \
                    -vARG1="$DATASET_DIR",ARG2="$local_unique_name",ARG3="$WD",ARG4="$data_type",ARG5="$length",ARG6="$TYPE",ARG7="$REV",ARG8="$VERBOSE" "$WD"/energy_measure/iqtree/test_script_iqtree.sh
          fi
      ...
      if [ "$IQTREE_OPENMP" == true ]; then
         ...
         qsub -P${PROJECT_NAME} -lwalltime=$omp_wall_time,ncpus=$IQTREE_THREADS,mem="${memory}GB",jobfs=10GB,wd -qnormal -N energy_iqtree_omp_${TYPE} \
                -vARG1="$DATASET_DIR",ARG2="$local_unique_name",ARG3="$WD",ARG4="$data_type",ARG5="$length",ARG6="$IQTREE_THREADS",ARG7="$TYPE",ARG8="$REV",ARG9="$VERBOSE" "$WD"/energy_measure/iqtree/test_script_iqtree_omp.sh
      fi
  done
done
