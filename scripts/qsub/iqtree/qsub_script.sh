      ...
      if [ "$V100_GPU" == true ]; then
        memory=$((factor * 1 * 48))
          qsub -P${PROJECT_NAME} -lwalltime=$wall_time,ncpus=12,ngpus=1,mem="${memory}GB",jobfs=10GB,wd -qgpuvolta -N test_v100 \
                -vARG1="$DATASET_DIR",ARG2="$local_unique_name",ARG3="$WD",ARG4="$data_type",ARG5="$length",ARG6="$TYPE",ARG7="$IQTREE_ARGS",ARG8="$NUM_TREES" "$WD"/test/iqtree/test_script_iqtree.sh
...
#      fi
      ...
      if [ "$IQTREE_OPENMP" == true ]; then
         ...
         qsub -P${PROJECT_NAME} -lwalltime=$wall_time,ncpus=$IQTREE_THREADS,mem="${memory}GB",jobfs=10GB,wd -qnormal -N test_iqtree_omp \
                -vARG1="$DATASET_DIR",ARG2="$omp_unique",ARG3="$WD",ARG4="$data_type",ARG5="$length",ARG6="$IQTREE_THREADS",ARG7="$IQTREE_AUTO",ARG8="$IQTREE_ARGS",ARG9="$NUM_TREES" "$WD"/test/iqtree/test_script_iqtree_omp.sh
      fi
  done
done
