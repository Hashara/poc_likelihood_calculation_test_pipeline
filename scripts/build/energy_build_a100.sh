...
mkdir -p "$WD/openacc_a100${suffix}"
...
cmake -DCMAKE_CXX_FLAGS="$LDFLAGS $CPPFLAGS" -DTARGET_A100=ON -DUSE_OPENACC=ON -DENERGY_MEASUREMENT=ON $code_dir
make VERBOSE=1 -j
...
#cmake -DCMAKE_CXX_FLAGS="$LDFLAGS $CPPFLAGS" -DUSE_OPENACC=ON -DTRANSPOSED_RATE_MATRIX=ON -DTARGET_A100=ON -DPROFILE=ON $code_dir
#make VERBOSE=1 -j