...
cd "$WD/openacc_h200" || { echo "Failed to change directory to openacc_h200"; exit 1; }
...
cmake -DCMAKE_CXX_FLAGS="$LDFLAGS $CPPFLAGS" -DUSE_OPENACC=ON -DTARGET_H200=ON $code_dir
make VERBOSE=1 -j
