            ...
            for type in "${executable_type[@]}"; do
                ...
                if [ -f "$executable_path" ]; then
                    ...
                    if [ "$AA_or_DNA" = "AA" ]; then
                        ...
                        nsys profile --trace=cuda,openacc,nvtx,cudaProfilerApi --sample=cpu,gpu --gpu-metrics-device=all --cuda-memory-usage=true --stats=true -o profile_report_${ex_type} $executable_path -s alignment_${length}.phy -te tree_${i}.full.treefile --seqtype AA -prefix output_${UNIQUE_NAME}_${taxa_size}_${length}_aa_${type}.txt
                        ...
                        ncu --set full -f -o ncu_report_${ex_type} $executable_path -s alignment_${length}.phy -te tree_${i}.full.treefile -prefix outputncu_${UNIQUE_NAME}_${taxa_size}_${length}_dna_${type}.txt
                    fi
