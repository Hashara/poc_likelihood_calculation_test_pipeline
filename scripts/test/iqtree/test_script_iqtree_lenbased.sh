          ...
          for type in "${executable_type[@]}"; do
              ...
              if [ -f "$executable_path" ]; then
                  ...
                  if [ "$AA_or_DNA" = "AA" ]; then
                      ...
                      $executable_path -s alignment_${length}.phy $tree_args --prefix "$(resolve_out_prefix "$TAXA_DIR" "output_${UNIQUE_NAME}_${taxa_size}_${length}_aa_${type}")" ${IQTREE_ARGS}

                  elif [ "$AA_or_DNA" = "DNA" ]; then
                      echo "Using DNA data"
                      $executable_path -s alignment_${length}.phy $tree_args --prefix "$(resolve_out_prefix "$TAXA_DIR" "output_${UNIQUE_NAME}_${taxa_size}_${length}_${type}")" ${IQTREE_ARGS}
